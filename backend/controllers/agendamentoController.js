...
exports.criar = async (req, res) => {
    ...
    try {
        // Tentar via RPC atômico no Supabase se existir
        ...
        // Fallback defensivo com verificação de concorrência:
        ...
        // Atualizar o contador de vagas ocupadas com proteção
        await supabase
            .from('disponibilidades')
...
exports.cancelar = async (req, res) => {
    ...
    try {
        ...
        if (agendamento.status !== 'agendado') {
            return res.status(400).json({ erro: `Não é possível cancelar um agendamento com status '${agendamento.status}'.` });
        }
        ...
        // Liberta a vaga na tabela de disponibilidades com proteção contra underflow
...
exports.adminCancelar = async (req, res) => {
    ...
    try {
        ...
        if (agendamento.status === 'cancelado') {
            return res.status(400).json({ erro: 'Este agendamento já se encontra cancelado.' });
        }

        // O Admin cancela sem verificar dono e sem verificar a regra das 2 horas!
        await supabase.from('agendamentos').update({ status: 'cancelado' }).eq('id', id);

        // Decrementa apenas se o agendamento estava ativo e evita valores negativos
        if (agendamento.status === 'agendado' && agendamento.disponibilidades?.id) {
            const vagasRestantes = Math.max(0, ((agendamento.disponibilidades?.vagas_ocupadas) || 1) - 1);
            await supabase
                .from('disponibilidades')
                .update({ vagas_ocupadas: vagasRestantes })
